...
<div class="content" onclick="remove_class()">
    ...
    <div class="inner_content">
        ...
        <form method="post" action="<?php echo $_SERVER['PHP_SELF']; ?>" class="hr_book_form">
            ...
            if(mysqli_num_rows($record) > 0)
            {
                while ($row = mysqli_fetch_assoc($record))
                {
                    ...
                    if(isset($_POST['submit'])) {
                        ...
                        $insert ="INSERT INTO oreservation(no_of_guest,slot_number, name, date_res, time, suggestions) VALUES('$guest', '$slot','$name','$d','$time', '$suggestions')";
            ...
            $query= "SELECT time FROM workingtime where time not in(select time from oreservation where date_res='$d')" ;
            ...
            $query="SELECT slot_number FROM addslot where dining_type='Outside-dining' and status='Available' and slot_number not in(select slot_number from oreservation where date_res='$d')" ;
